...
class o1NeuroBoost:
    ...
    def train_reboost(self, X, y, b=100, bootstrap=True, K = None):
        M = len(self.sparseneuros)
        self.X0 = X.copy()
        self.y_mean = np.mean(y, axis = 0)
        self.y0 = y.copy() - self.y_mean
        ...
        n_iterations = b
        for i, network in enumerate(self.sparseneuros):
            # Transfter learning:
    ...
    def train_stable(self, X, y, test_size = 0.2, b = 100):
        ...
        self.sparseneuros = networks
        M = len(self.sparseneuros)
        # Calculate the residuals based on the input (X, y)
        residuals = [np.zeros(len(y0_temp)) for _ in range(M)]
        residuals[0] = y0_temp
        self.sparseneuros[0].y0 = residuals[0]

        for l, network in enumerate(self.sparseneuros):
    ...
    def copy(self):
        ...
        networks = [o1Neuro(self.p, 
                            ...
                            s0 = self.s0,
                            X_neighbor = self.X_neighbor
                            ) for i in range(M)]
        for l, network in enumerate(networks):
            # Copy weights and biases
            # Do not copy residuals
            network.W_star = self.sparseneuros[l].W_star
            network.C_star = self.sparseneuros[l].C_star
        return networks
    ...
    def train_bagging(self, b = 10, n_estimators = 5):
        ...
        for i in range(n_estimators):
            ...
            model = o1NeuroBoost(self.p, 
                                 ...
                                 sparsity_level=2,
                                 stochastic_ratio = self.stochastic_ratio)
            model.sparseneuros = self.copy()            
            model.train_stable(X_boot, y_boot, b = b)
            models.append(model)
