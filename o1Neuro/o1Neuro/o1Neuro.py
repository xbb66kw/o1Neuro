        ...
        for i_, weight_matrix in enumerate(reversed(W_star)):    
            ...
            for r in range(n_):
                ...
                if connected_:
                    ...
                    i_, j_ = X_train.shape
                    j2, l_, k_ = W[0].shape
                    assert j_ == j2, "Dimension mismatch"
                    
                    out = np.empty((i_, l_, k_))
                    for dim in range(k_):
                        out[..., dim] = X_train @ W[0][..., dim]
                    W_out = [out > C[0]]
                    ...
                    for t in range(1, len(p) - 1):
                        ...
                        # Transpose to bring `k` to front
                        A_ = np.transpose(W_out[t-1], (2, 0, 1))  # shape (k, i, j)
                        B_ = np.transpose(W[t], (2, 1, 0))        # shape (k, l, j)
                        
                        # Batched matmul: (k, i, j) @ (k, j, l) => (k, i, l)
                        C_ = np.matmul(A_, B_.transpose(0, 2, 1))
                        
                        # Transpose back to (i, l, k)
                        W_out.append(np.transpose(C_, (1, 2, 0)) > C[t])
