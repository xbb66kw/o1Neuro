...
for type_ in [0, 2]:
    for r in range(R):
        ...
        X_train = np.random.uniform(-2*np.pi, 2*np.pi, size=(n, p))
        
    
        if type_ == 0:
            # Polynomial Regression
            y_train = 2 * X_train[:, 0]**2 - 3 * X_train[:, 1] + 0.5 * X_train[:, 2] + np.random.randn(n)
        elif type_ == 2:
            # Multiplicative Interaction
            y_train = X_train[:, 0] * X_train[:, 1] + 0.5 * X_train[:, 2] + np.random.randn(n)
        ...
        X_test = np.random.uniform(-2*np.pi, 2*np.pi, size=(n_test, p))
        ...
        if run_o1neuro:
            ...
            o1neuro.train_stable(X_train,
                                 y_train, 
                                 b = int(o1neuro_param['b']),
                                 test_size = o1neuro_param["test_size"])
            o1neuro.train_bagging(b = 10, n_estimators = 20)
            
            end_time = time.time()
            
        
            y_pred_dspione_bagging = o1neuro.bagging_predict(X_test)
